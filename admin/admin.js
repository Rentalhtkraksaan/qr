...
function updateSelectedCount() {
    const countEl = document.getElementById('selected-qr-count');
    if (!countEl) return;
    const checked = document.querySelectorAll('.chk-select-qr:checked').length;
...
function renderAdminQrTable() {
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color: var(--text-dim);">Tidak ada data QR ditemukan.</td></tr>`;
        updateSelectedCount();
        return;
    }
    ...
    tbody.innerHTML = filtered.map(q => {
        ...
        else if (q.status === 'disabled') statusBadge = '<span class="badge" style="background:rgba(239,68,68,0.2); color:#fca5a5;">Dimatikan</span>';

        return `
            <tr>
                <td style="text-align:center;"><input type="checkbox" class="chk-select-qr" data-id="${q.id}"></td>
                ...
                <td>${q.googleUrl ? `<a href="${q.googleUrl}" target="_blank" style="color:var(--accent-cyan);"><i class="fa-brands fa-google"></i> Buka Link Review</a>` : '<em style="color:var(--text-dim)">-</em>'}</td>
                ...
                <td>
                    <div style="display:flex; gap:6px; flex-wrap:wrap;">
                        ...
                        <button class="btn btn-glass-danger btn-sm btn-delete-single" data-id="${q.id}" title="Hapus QR Code"><i class="fa-solid fa-trash"></i> Hapus</button>
                    </div>
                </td>
            </tr>
        `;
    ...
    tbody.querySelectorAll('.btn-delete-single').forEach(btn => {
        btn.addEventListener('click', (e) => deleteSingleQr(e.currentTarget.getAttribute('data-id')));
    });

    const chkAll = document.getElementById('chk-select-all-qr');
    if (chkAll) {
        chkAll.checked = false;
        chkAll.onclick = function() {
            tbody.querySelectorAll('.chk-select-qr').forEach(cb => cb.checked = chkAll.checked);
            updateSelectedCount();
        };
    }

    tbody.querySelectorAll('.chk-select-qr').forEach(cb => {
...
function deleteSingleQr(qrId) {
    const qr = adminState.data.qrs.find(q => q.id === qrId);
    if (!qr) return;

    const cafeText = qr.cafeName ? ` (Kafe: "${qr.cafeName}")` : '';
    const confirmDelete = confirm(`⚠️ KONFIRMASI HAPUS:\n\nApakah Anda YAKIN ingin menghapus QR Code "${qr.id}"${cafeText} secara PERMANEN?\n\nData ini akan dihapus total dari database dan tidak dapat dikembalikan lagi.`);
    if (!confirmDelete) return;

    adminState.data.qrs = adminState.data.qrs.filter(q => q.id !== qrId);
    if (db) db.ref(`qrs/${qrId}`).remove().catch(e => {});

    saveLocalState();
    renderAdminDashboard();
    alert(`✅ QR Code "${qrId}" BERHASIL DIHAPUS.`);
}

function deleteBulkQrs() {
    const checkedBoxes = document.querySelectorAll('.chk-select-qr:checked');
    if (checkedBoxes.length === 0) {
        alert('❌ Silakan pilih/centang minimal 1 QR Code yang ingin dihapus terlebih dahulu!');
        return;
    }

    const idsToDelete = Array.from(checkedBoxes).map(cb => cb.getAttribute('data-id'));
    const confirmBulk = confirm(`⚠️ KONFIRMASI HAPUS MASSAL:\n\nApakah Anda YAKIN ingin menghapus ${idsToDelete.length} QR Code yang dipilih secara PERMANEN?\n\nSemua data QR terpilih ini akan dihapus total dari database.`);
    if (!confirmBulk) return;

    idsToDelete.forEach(id => {
        adminState.data.qrs = adminState.data.qrs.filter(q => q.id !== id);
        if (db) db.ref(`qrs/${id}`).remove().catch(e => {});
    });

    saveLocalState();
    renderAdminDashboard();
    alert(`✅ BERHASIL! ${idsToDelete.length} QR Code telah dihapus secara massal.`);
}
...
function openPrintModal(qrId) {
    ...
    if (canvasEl) {
        renderStyledQrCode(canvasEl, fullPublicUrl, 128);
    }
...
function setupAdminEventListeners() {
    ...
    if (formWebReg) {
        formWebReg.addEventListener('submit', (e) => {
            ...
            const rawGoogleUrl = document.getElementById('web-google-url').value;
    ...
    // Bulk Delete Button
    const btnBulkDelete = document.getElementById('btn-bulk-delete-qr');
    if (btnBulkDelete) btnBulkDelete.addEventListener('click', deleteBulkQrs);
    ...
    // Setup Place Search Autocomplete for Admin
    setupPlaceSearchAutocomplete();

    document.querySelectorAll('.close-modal').forEach(btn => btn.addEventListener('click', closeModals));
}

// Smart Location & Place Autocomplete Engine for Admin
function setupPlaceSearchAutocomplete() {
    const inputEl = document.getElementById('web-cafe-name');
    const dropdownEl = document.getElementById('web-cafe-autocomplete-dropdown');
    const badgeEl = document.getElementById('web-cafe-verified-badge');
    const badgeTextEl = document.getElementById('web-verified-place-text');
    const googleUrlInput = document.getElementById('web-google-url');
    ...
    inputEl.addEventListener('input', (e) => {
        ...
        debounceTimer = setTimeout(() => {
            fetchPlaceSuggestions(query, dropdownEl, (selectedPlace) => {
                inputEl.value = selectedPlace.name;
                
                const directUrl = selectedPlace.reviewUrl || `https://search.google.com/local/writereview?query=${encodeURIComponent(selectedPlace.name)}`;
                if (googleUrlInput) googleUrlInput.value = directUrl;

                if (badgeEl && badgeTextEl) {
                    badgeTextEl.innerHTML = `<strong>Bener Tempat Ini? YES:</strong> "${selectedPlace.name}" ${selectedPlace.address ? `(${selectedPlace.address})` : ''}`;
...
function fetchPlaceSuggestions(query, dropdownEl, onSelectCallback) {
    const qLower = query.toLowerCase();

    let results = (adminState.data.qrs || [])
        .filter(q => q.cafeName && q.cafeName.toLowerCase().includes(qLower))
        .map(q => ({
            name: q.cafeName,
            address: "Terdaftar di Database System",
            reviewUrl: q.googleUrl
        }));

    results.push(
        {
            name: query.toUpperCase(),
            address: `Hasil Utama Google Maps Review Bintang 5 untuk "${query}"`,
            reviewUrl: `https://search.google.com/local/writereview?query=${encodeURIComponent(query)}`
        },
        {
            name: `${query} Official`,
            address: `Google Business Profile Resmi Ulasan Bintang 5`,
            reviewUrl: `https://search.google.com/local/writereview?query=${encodeURIComponent(query + ' Official')}`
        }
    );

    dropdownEl.innerHTML = results.map((item, idx) => `
        <div class="maps-dropdown-item" data-idx="${idx}">
            <div class="maps-dropdown-icon"><i class="fa-solid fa-location-dot"></i></div>
            <div>
                <div class="maps-dropdown-title">${item.name}</div>
                <div class="maps-dropdown-sub">${item.address}</div>
            </div>
        </div>
    `).join('');
    ...
    dropdownEl.querySelectorAll('.maps-dropdown-item').forEach(el => {
